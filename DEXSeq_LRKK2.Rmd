...
```{r}
txtDirectory = "/Volumes/My_Passport/Transcriptomics/LRKK2_Clean/"
...
groups_file_LRKK2 <- read.delim("/Volumes/My_Passport/Transcriptomics/LRKK2_Clean/groups_file_LRKK2.txt", header=FALSE)

sampleTable = data.frame(
   row.names = groups_file_LRKK2$V1,
   condition = groups_file_LRKK2$V2 )
...
dxd = estimateExonFoldChanges( dxd, fitExpToVar="condition") #to calculate fold changes, optional
...
plotDEXSeq( dxr1, "ENSG00000119685", legend=TRUE, cex.axis=1.2, cex=1.3, lwd=2 )
...
LRKK2_list_DEXSeq <- data.frame(GeneID=dxr1@listData[["groupID"]], Exon_Bin=dxr1@listData[["featureID"]], Start=dxr1@listData[["genomicData"]]@ranges@start, width=dxr1@listData[["genomicData"]]@ranges@width, Mean_Counts=dxr1@listData[["exonBaseMean"]], FDR=dxr1@listData[["padj"]], log2fold_change=dxr1@listData[["log2fold_WT_LRKK2"]]) %>%
  filter(FDR<=0.05) %>%
 # tibble::rownames_to_column("GeneID") %>%
  drop_na() %>%
  #mutate(log2fold_change=(log2fold_change * -1)) %>% # invert the sign of fold change so it's LRRK2/WT
...
write.csv(LRKK2_list_DEXSeq,'LRKK2_DEXSeq_0.05.csv', row.names = F)
...
ensembl_gene_names <- ensembl_gene_names %>%
  ...
  ensembl_gene_names <- rbind(ensembl_gene_names, c("ENSG00000215301", "DDX3X")) # adding genes on chr X
...
#Volcano_list <- mutate(Volcano_list, log2fold_change=(log2fold_change * -1)) # invert the sign of fold change so it's LRRK2/WT
...
LRKK2_final_list <- read_csv("/Users/eugeniograceffo/Desktop/Oxford/2nd_Rotation/Transcriptomics/LRKK2_0.05.csv")

 volcano_plot <- EnhancedVolcano(Volcano_list_genes,
    lab = Volcano_list_genes$Gene ,
    x = 'log2fold_change',
    y = 'FDR',
    #selectLab = LRKK2_final_list$genes ,
    selectLab = c('IFT43','TTLL5', 'ZNF320', 'GAS5', 'NIN', 'DDX3X', 'SRSF11'), #for specific genes 
    pCutoff = 0.05,
    FCcutoff = 0,
    xlim = c(-20, 20), # 5.5
...
ggsave("LRKK2_Volcano.pdf", width = 40, height = 25, units = "cm")
```
